...
(function(window) {
    ...
    const GantryFormula = {
        evaluate(expression) {
            ...
            const toNum = (value) => isFinite(Number(value)) ? Number(value) : 0;
            const toBool = (value) => toNum(value) !== 0;
            ...
            const parseLogicalOr = () => {
                ...
                while (match('operator', '||')) value = toBool(value) || toBool(parseLogicalAnd());
                return value;
            };
            const parseLogicalAnd = () => {
                let value = parseEquality();
                while (match('operator', '&&')) value = toBool(value) && toBool(parseEquality());
                return value;
            };
            const parseEquality = () => {
                let value = parseComparison();
                while (true) {
                    if (match('operator', '==')) value = toNum(value) === toNum(parseComparison());
                    else if (match('operator', '!=')) value = toNum(value) !== toNum(parseComparison());
                    else return value;
                }
            };
            const parseComparison = () => {
                ...
                while (true) {
                    if (match('operator', '>=')) value = toNum(value) >= toNum(parseAdditive());
                    else if (match('operator', '<=')) value = toNum(value) <= toNum(parseAdditive());
                    else if (match('operator', '>')) value = toNum(value) > toNum(parseAdditive());
                    else if (match('operator', '<')) value = toNum(value) < toNum(parseAdditive());
                    else return value;
                }
            ...
            const parseAdditive = () => {
                ...
                while (true) {
                    if (match('operator', '+')) value = toNum(value) + toNum(parseMultiplicative());
                    else if (match('operator', '-')) value = toNum(value) - toNum(parseMultiplicative());
                    else return value;
                }
            };
            const parseMultiplicative = () => {
                ...
                while (true) {
                    if (match('operator', '*')) value = toNum(value) * toNum(parseUnary());
                    else if (match('operator', '/')) {
                        const divisor = toNum(parseUnary());
                        value = divisor === 0 ? 0 : toNum(value) / divisor;
                    } else if (match('operator', '%')) {
                        const divisor = toNum(parseUnary());
                        value = divisor === 0 ? 0 : toNum(value) % divisor;
                    } else return value;
            ...
            const parseUnary = () => {
                if (match('operator', '!')) return !toBool(parseUnary());
                if (match('operator', '+')) return toNum(parseUnary());
                if (match('operator', '-')) return -toNum(parseUnary());
                return parsePrimary();
            };
            const parsePrimary = () => {
            ...
            if (index !== tokens.length) throw new Error('Unexpected trailing content');
            return result;
        },
        ...
        tokenize(source) {
            ...
            while (index < source.length) {
                ...
                if (/[0-9.]/.test(char)) {
                    ...
                    tokens.push({ type: 'number', value: parseFloat(source.substring(index, end)) });
                    index = end;
                    continue;
                }
        ...
        executeFunction(name, args) {
            const values = args.map(function(arg) { return Number(arg) || 0; });
            switch (name) {
                case 'min': return values.length ? Math.min.apply(Math, values) : 0;
                case 'max': return values.length ? Math.max.apply(Math, values) : 0;
        ...
        generate(value, options = {}) {
            ...
            if (!isFinite(Number(value))) {
                ...
                <div class="health-gauge-container" style="width:${size}px; text-align:center;">
                    <div class="d-flex align-items-center justify-content-center rounded-circle border text-muted"
                         style="width:${size}px; height:${size}px; border-color:#e5e7eb !important; font-weight:700;">
                        —
                    </div>
        ...
        generateSemi(value, options = {}) {
            ...
            if (!isFinite(Number(value))) {
                ...
                <svg width="${width}" height="${height}" class="health-gauge-semi">
                    <path d="M ${strokeWidth/2} ${height - strokeWidth / 2} A ${(width - strokeWidth) / 2} ${(width - strokeWidth) / 2} 0 0 1 ${width - strokeWidth/2} ${height - strokeWidth / 2}"
                          fill="none" stroke="#e5e7eb" stroke-width="${strokeWidth}" stroke-linecap="round"/>
                    ${showValue ? `<text x="${width / 2}" y="${height - 10}" text-anchor="middle" font-size="${width * 0.2}px" font-weight="700" fill="#6b7280">—</text>` : ''}
                </svg>`;
            }
    ...
    // ACCESS STATE
    // ==========================================
    // Gantry is open source with no license or activation. This permissive stub
    // replaces the former license manager so the UI never gates on entitlement.
    // Method names are preserved for backward compatibility with existing callers.
    const Access = {
        _data: { valid: true, status: 'community' },
    ...
    window.GantryFormula = GantryFormula;
    window.API = API;
    ...
    // Access state (no license/activation in the open source build)
    window.Access = Access;
    window.License = Access; // backward-compatible alias
