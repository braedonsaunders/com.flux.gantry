    ...
    window.CustomerValueController = {
        ...
        renderSegmentCustomersTable: function(customers) {
            ...
            pageItems.forEach(function(c) {
                ...
                html += '<td class="text-center">' + Sparkline.generate(trendData, { width: 50, height: 16 }) + '</td>';
                html += '</tr>';
            });
        ...
        getSparklineData: function(customer) {
            ...
            // Generate varied data based on customer metrics
            var total = customer.totalRevenue || customer.historicalCLV || 0;
            var orders = customer.orderCount || customer.transactionCount || 1;
            var avgOrder = total / Math.max(orders, 1);
            var daysSince = customer.daysSinceLast || 0;
            var retention = customer.retentionFactor || 50;
            var tier = (customer.tier || '').toLowerCase();
            var segment = (customer.segment || customer.rfmSegment || '').toLowerCase();
            
            // Use customer ID to create consistent but varied patterns
            var customerId = parseInt(customer.customerId) || Math.random() * 10000;
            var seed = customerId % 6; // 0-5 pattern selection
            
            var trend;
            
            // First check segment if available
            if (segment === 'champions' || segment === 'loyal') {
                trend = [0.6, 0.7, 0.75, 0.85, 0.9, 1.0];
            } else if (segment === 'at-risk' || segment === 'hibernating' || segment === 'lost') {
                trend = [1.0, 0.9, 0.7, 0.5, 0.3, 0.1];
            } else if (segment === 'new') {
                trend = [0, 0, 0.1, 0.3, 0.7, 1.0];
            } else {
                // Use tier and retention to determine pattern
                if (tier === 'platinum' || tier === 'gold' || retention >= 70) {
                    // Growing trends for high-value customers
                    var growthPatterns = [
                        [0.5, 0.6, 0.7, 0.8, 0.85, 1.0],
                        [0.4, 0.5, 0.65, 0.75, 0.9, 1.0],
                        [0.6, 0.55, 0.7, 0.8, 0.95, 1.0]
                    ];
                    trend = growthPatterns[seed % 3];
                } else if (daysSince > 90 || retention < 40) {
                    // Declining for inactive customers
                    var declinePatterns = [
                        [1.0, 0.85, 0.7, 0.5, 0.35, 0.2],
                        [0.9, 0.8, 0.6, 0.45, 0.3, 0.15],
                        [1.0, 0.9, 0.75, 0.55, 0.4, 0.25]
                    ];
                    trend = declinePatterns[seed % 3];
                } else {
                    // Varied stable patterns for others
                    var stablePatterns = [
                        [0.5, 0.6, 0.55, 0.65, 0.6, 0.7],
                        [0.6, 0.5, 0.7, 0.55, 0.65, 0.6],
                        [0.7, 0.65, 0.6, 0.7, 0.65, 0.75],
                        [0.55, 0.7, 0.6, 0.55, 0.7, 0.65],
                        [0.6, 0.65, 0.55, 0.7, 0.6, 0.65],
                        [0.5, 0.55, 0.6, 0.5, 0.55, 0.6]
                    ];
                    trend = stablePatterns[seed];
                }
            }
            
            // Scale by average order value
            return trend.map(function(t) { return Math.round(avgOrder * t); });
        },
        ...
        renderCLV: function() {
            ...
            if (profitMethod === 'estimated' || profitMethod === 'estimated_fallback') {
                profitSubLabel += ' (est)';
            ...
            if (profitMethod === 'estimated' || profitMethod === 'estimated_fallback') {
                html += '<div class="alert alert-info small mb-3"><i class="fas fa-info-circle mr-2"></i>';
                html += profitMethod === 'estimated_fallback' 
                    ? 'No project data found. Using estimated margin (' + (profitSummary.avgMarginPct || 35) + '%). Configure in Settings tab.'
                    : 'Using estimated margin (' + (profitSummary.avgMarginPct || 35) + '%). Switch to Project Financials in Settings for actual profitability.';
                html += '</div>';
            } else if (profitMethod === 'gl_rollup_fallback') {
                html += '<div class="alert alert-info small mb-3"><i class="fas fa-info-circle mr-2"></i>';
                html += 'ProjectFinancials table empty. Using GL Rollup as fallback.';
                html += '</div>';
            } else if (profitMethod === 'disabled') {
        ...
        renderProfitabilityTab: function() {
            ...
            // Method indicator with better fallback messaging
            var methodClass = method === 'project_financials' ? 'success' : 
                              method.includes('fallback') ? 'warning' : 'info';
            html += '<div class="alert alert-' + methodClass + ' py-2 mb-3">';
            html += '<i class="fas fa-' + (method === 'project_financials' ? 'check-circle' : 'info-circle') + ' mr-2"></i>';
            html += '<strong>Method:</strong> ' + this.formatMethodName(method);
            
            if (method === 'gl_rollup_fallback') {
                html += '<br><small class="text-muted">ProjectFinancials table returned no data for this period. ';
                html += 'This may occur if: (1) Project Accounting is not enabled, (2) No jobs have ProjectFinancials entries, ';
                html += 'or (3) No jobs were created within the analysis date range. ';
                html += 'Using GL Rollup method instead which calculates profitability from transaction lines.</small>';
            } else if (method === 'estimated_fallback') {
                html += '<br><small class="text-muted">No job cost data available. Using estimated margin from Configuration tab.</small>';
        ...
        formatMethodName: function(method) {
            var names = {
                ...
                'gl_rollup': 'GL Rollup',
                'gl_rollup_fallback': 'GL Rollup (Fallback)',
                'estimated': 'Estimated Margin',
                'estimated_fallback': 'Estimated Margin (Fallback)',
                'disabled': 'Disabled'
            };
        ...
        renderConfigTab: function() {
            ...
            html += this.configSelect('Calculation Method', 'profitabilityMethod', config.profitabilityMethod || 'project_financials', 'How to calculate gross profit', [
                ...
                { value: 'estimated', label: 'Estimated Margin (applies fixed %)' },
                { value: 'disabled', label: 'Disabled' }
            ]);
            html += this.configInput('Estimated Margin %', 'profitabilityEstimatedMarginPct', config.profitabilityEstimatedMarginPct || 35, 'Default margin for estimated method or fallback', 'number', 0, 100);
            html += '</div>';
        ...
        saveConfig: function() {
            ...
            var config = {
                ...
                profitabilityEstimatedMarginPct: parseInt(el('#cv_profitabilityEstimatedMarginPct').value) || 35,
