...
function(query, search, runtime, format, log, ConfigLib, Utils) {
    ...
    function normalizeDateString(value) {
        ...
        const normalized = formatDateForQuery(value);
        ...
        return formatDateForQuery(parsed) === normalized ? normalized : null;
    }

    /**
     * Normalize an integer identifier
     * @param {*} value
    ...
    /**
     * Normalize an array of integer identifiers
     * @param {*} values
     * @param {Object} [options]
     * @returns {number[]}
     */
    function normalizeIntegerList(values, options = {}) {
        ...
        return source
            .map(function(value) { return normalizeInteger(value, options); })
            .filter(function(value) { return value !== null; });
    }

    /**
     * Build the fiscal-year start date for a reference date
     * @param {Date} referenceDate
     * @param {number} startMonth - 0-based month
     * @param {number} startDay - 1-based day
     * @returns {Date}
     */
    function buildFiscalYearStart(referenceDate, startMonth, startDay) {
        const ref = referenceDate instanceof Date ? new Date(referenceDate) : new Date(referenceDate);
        const month = normalizeInteger(startMonth, { min: 0, max: 11 });
        const day = normalizeInteger(startDay, { min: 1, max: 31 }) || 1;
        const fiscalMonth = month === null ? 0 : month;

        let fiscalYear = ref.getFullYear();
        if (ref.getMonth() < fiscalMonth || (ref.getMonth() === fiscalMonth && ref.getDate() < day)) {
            fiscalYear -= 1;
        }

        return new Date(fiscalYear, fiscalMonth, day);
    }
    ...
    function buildSubsidiaryFilter(subsidiaryId, tableAlias) {
        const normalized = normalizeInteger(subsidiaryId, { min: 1 });
        if (normalized === null) return '';
        
        const prefix = tableAlias ? tableAlias + '.' : '';
        return ` AND ${prefix}subsidiary = ${normalized}`;
    }

    /**
     * Evaluate a constrained numeric expression without using eval/new Function.
     * Supports arithmetic, comparisons, logical operators, ternary expressions,
     * and a small set of math helpers.
     *
    ...
    function evaluateFormula(expression) {
        ...
        function parseLogicalOr() {
            ...
            while (match('operator', '||')) {
                value = toBoolean(value) || toBoolean(parseLogicalAnd());
            }
            return value;
        }
        ...
        function parseEquality() {
            ...
            while (true) {
                if (match('operator', '==')) {
                    value = toNumber(value) === toNumber(parseComparison());
        ...
        function parseComparison() {
            ...
            while (true) {
                if (match('operator', '>=')) {
                    value = toNumber(value) >= toNumber(parseAdditive());
                } else if (match('operator', '<=')) {
                    value = toNumber(value) <= toNumber(parseAdditive());
                } else if (match('operator', '>')) {
                    value = toNumber(value) > toNumber(parseAdditive());
        ...
        function parseAdditive() {
            ...
            while (true) {
                if (match('operator', '+')) {
                    value = toNumber(value) + toNumber(parseMultiplicative());
                } else if (match('operator', '-')) {
                    value = toNumber(value) - toNumber(parseMultiplicative());
                } else {
                    return value;
        ...
        function parsePrimary() {
            ...
            if (match('identifier')) {
                ...
                throw new Error('Unsupported formula token: ' + token.value);
            }
        ...
        if (position !== tokens.length) {
            throw new Error('Unexpected trailing formula content');
        }
    ...
    function tokenizeFormula(source) {
        ...
        while (index < source.length) {
            ...
            const multiOperator = ['>=', '<=', '==', '!=', '&&', '||'].indexOf(twoChar) !== -1;
            if (multiOperator) {
            ...
            if (/[0-9.]/.test(char)) {
                ...
                const raw = source.substring(index, end);
                const number = parseFloat(raw);
                if (!isFinite(number)) {
                    throw new Error('Invalid numeric token in formula');
                }
    ...
    function executeFormulaFunction(name, args) {
        const numericArgs = args.map(function(arg) { return toNumber(arg); });
        switch (name) {
            case 'min':
                return numericArgs.length ? Math.min.apply(Math, numericArgs) : 0;
            case 'max':
                return numericArgs.length ? Math.max.apply(Math, numericArgs) : 0;
            case 'ceil':
                return Math.ceil(numericArgs[0] || 0);
            case 'floor':
                return Math.floor(numericArgs[0] || 0);
            case 'round':
                return Math.round(numericArgs[0] || 0);
            case 'sqrt':
                return Math.sqrt(numericArgs[0] || 0);
            case 'pow':
                return Math.pow(numericArgs[0] || 0, numericArgs[1] || 0);
            case 'abs':
                return Math.abs(numericArgs[0] || 0);
            case 'avg':
                return numericArgs.length
                    ? numericArgs.reduce(function(sum, value) { return sum + value; }, 0) / numericArgs.length
                    : 0;
            default:
    ...
    return {
        ...
        normalizeIntegerList: normalizeIntegerList,
        buildFiscalYearStart: buildFiscalYearStart,
