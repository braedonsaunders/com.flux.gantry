...
define(['N/query', 'N/runtime', 'N/search', 'N/log', './Lib_Core', './advisor/Lib_Advisor_Utils'], function(query, runtime, search, log, Core, Utils) {
    ...
    const runSuiteQLPaginated = Core.runQueryPaginated;
    ...
    function getCustomerTransactionData(startDate, endDate, subsidiaryId, config, diagnostics) {
        ...
        try {
            const rawResults = runSuiteQLPaginated(sql);
        ...
        if (method === 'estimated') {
            return {
                summary: {
                    method: 'unsupported',
                    status: 'error',
                    message: 'Estimated profitability is disabled in exact mode. Select Project Financials, GL Rollup, or Disabled.'
                },
                customers: []
            };
        }
            ...
            if (!result.customers || result.customers.length === 0) {
                return {
                    summary: {
                        ...
                        status: 'error',
                        message: 'Project Financials returned no profitability data for the selected jobs.',
                        diagnostic: result.summary ? result.summary.diagnostic : null
                    },
                    customers: []
                };
            }
        ...
        if (!result.customers || result.customers.length === 0) {
            return {
                summary: {
                    method: 'gl_rollup',
                    status: 'error',
                    message: 'GL Rollup returned no profitability data for the selected jobs.'
                },
                customers: []
            };
        }
    ...
    function calculateGLRollupProfitability(startDate, endDate, subsidiaryId, config) {
        ...
        try {
            const results = runSuiteQLPaginated(sql);
