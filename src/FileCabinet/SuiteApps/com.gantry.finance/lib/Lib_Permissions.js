...
define(['N/runtime', 'N/log', './Lib_Config'], function(runtime, log, ConfigLib) {
    ...
    function filterDashboards(dashboards) {
        const baseDashboards = dashboards.filter(function(dashboard) {
            return dashboard.id !== 'settings' || isAdmin();
        });
        const config = getPermissionsConfig();
        ...
        // Always include advisor as a utility dashboard
        const alwaysAllowed = ['advisor'];

        return baseDashboards.filter(d =>
