...
function(query, record, search, runtime, format, Core, Utils) {
    ...
    // ==========================================
    // HELPER: Safe Entity ID Extraction
    // NetSuite returns ScriptNullObjectAdapter for null values, which is truthy
    // but causes issues when used as object keys or serialized to JSON.
    // This helper ensures we get actual primitive values.
    // ==========================================

    function safeEntityId(value) {
        if (value === null || value === undefined) return null;
        // Convert to string to check for ScriptNullObjectAdapter pattern
        const str = String(value);
        if (str.indexOf('ScriptNullObjectAdapter') !== -1 || str === 'null' || str === 'undefined' || str === '') {
            return null;
        }
    ...
    function analyzeAuditTrail(startDate, endDate, subsidiaryId) {
        try {
            ...
            allRecords.forEach(function(record) {
                (record.users || []).forEach(function(userId) {
            ...
            for (var i = 0; i < results.length; i++) {
                ...
                if (timestamp) {
                    ...
                    if (!isNaN(noteDate.getTime())) {
                        isWeekend = noteDate.getDay() === 0 || noteDate.getDay() === 6;
                        if (isWeekend) {
                            weekendCount += 1;
                        }
                    }
    ...
    function handleRequest(params) {
        ...
        const startDate = normalizeDateInput(params.startDate, getDefaultStartDate());
        const endDate = normalizeDateInput(params.endDate, Core.getDefaultEndDate());
        const vendorId = normalizePositiveInt(params.vendorId);
        const userId = normalizePositiveInt(params.userId);
        const recordId = normalizePositiveInt(params.recordId);
        ...
        switch (subAction) {
            case 'audit_record_detail':
                if (!recordId) return invalid('Valid recordId is required');
                return {
                    status: 'success',
            ...
            case 'vendor_transactions':
                if (!vendorId) return invalid('Valid vendorId is required');
                return {
                    status: 'success',
            ...
            case 'user_transactions':
                if (!userId) return invalid('Valid userId is required');
                return {
                    status: 'success',
    ...
    function getVendorTransactions(vendorId, startDate, endDate, subsidiaryId) {
        const safeVendorId = normalizePositiveInt(vendorId);
        const safeStartDate = normalizeDateInput(startDate, getDefaultStartDate());
        const safeEndDate = normalizeDateInput(endDate, Core.getDefaultEndDate());
        const safeSubsidiaryId = normalizePositiveInt(subsidiaryId);
        if (!safeVendorId) return [];
        const subFilter = safeSubsidiaryId ? `AND T.subsidiary = ${safeSubsidiaryId}` : '';
    ...
    function getUserTransactions(userId, startDate, endDate, subsidiaryId) {
        const safeUserId = normalizePositiveInt(userId);
        const safeStartDate = normalizeDateInput(startDate, getDefaultStartDate());
        const safeEndDate = normalizeDateInput(endDate, Core.getDefaultEndDate());
        const safeSubsidiaryId = normalizePositiveInt(subsidiaryId);
