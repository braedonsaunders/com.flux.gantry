...
define(["N/query", "N/search", "N/log", "N/runtime", "./Lib_Core", "./Lib_Config", "./advisor/Lib_Advisor_Utils"], function (
    ...
    function calculateHeadcountCategoryData(cat, depts, allocationBases, config) {
        ...
        const avgSalary = parseFloat(config.avgSalary);
        const hasConfiguredSalary = isFinite(avgSalary) && avgSalary > 0;
        ...
        const result = {
            ...
            headcount: 0,
            warnings: [],
            avgSalaryUsed: hasConfiguredSalary ? avgSalary : null,
            usingDefaultSalary: false
        };

        if (!hasConfiguredSalary) {
            result.warnings.push('Headcount category requires avgSalary in settings. No expense was calculated.');
        }
        ...
        depts.forEach(d => {
            ...
            const expense = hasConfiguredSalary ? (hc * avgSalary) : 0;
    ...
    function calculateFormulaCategoryData(cat, categoryTotals, depts, allocationBases, config) {
        ...
        try {
            ...
            const calculatedExpense = Core.evaluateFormula(evalFormula);
    ...
    function getAccountTransactions(data) {
        const accountId = Core.normalizeInteger(data.accountId, { min: 1 });
        const startDate = Core.normalizeDateString(data.startDate);
        const endDate = Core.normalizeDateString(data.endDate);
        const page = Core.normalizeInteger(data.page, { min: 1 }) || 1;
        const pageSize = Core.normalizeInteger(data.pageSize, { min: 1, max: MAX_TRANSACTIONS }) || DEFAULT_PAGE_SIZE;

        if (!accountId || !startDate || !endDate) {
            return {
                accountId: accountId,
                transactions: [],
                pagination: { page: 1, pageSize: pageSize, totalCount: 0, totalPages: 0 },
                totals: { amount: 0, count: 0 },
                error: 'Valid accountId, startDate, and endDate are required'
            };
        }
    ...
    function getScoreOnly() {
        try {
            ...
            try {
                var burdenConfig = ConfigLib.getStoredConfiguration('burden') || {};
                var profile = getActiveProfile(burdenConfig);
                var allAccounts = loadAllExpenseAccounts();
                var classification = classifyAccounts(allAccounts, profile, profile.categories || []);
                totalAccounts = allAccounts.length - classification.excluded.length;
                unassignedCount = classification.unassigned.length;
            } catch (e) {
