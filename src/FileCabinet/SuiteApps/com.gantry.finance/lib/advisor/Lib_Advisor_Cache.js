 ...
 */
define(['N/cache', 'N/log', 'N/runtime', '../Lib_Dashboard_Registry', './Lib_Advisor_Utils'], function(cache, log, runtime, DashboardRegistry, Utils) {
    'use strict';
    ...
    const CACHE_SCOPE = cache.Scope.PRIVATE;
    ...
    function getCurrentOwnerContext() {
        ...
        return {
            ...
            roleId: String(currentUser.role || ''),
            accountId: String(runtime.accountId || '')
        };
    }

    function isOwnerMatch(owner) {
        if (!owner) return false;
        const current = getCurrentOwnerContext();
        return owner.userId === current.userId &&
            owner.roleId === current.roleId &&
    ...
    // Progress CRUD
    function progressCreate(requestId, message, agentState, requestContext) {
    ...
    function progressHasAccess(requestId) {
        const state = progressGet(requestId);
        return !!(state && isOwnerMatch(state.owner));
    }
    ...
    function progressGetPollingResponse(requestId) {
        ...
        if (!isOwnerMatch(state.owner)) {
            log.audit('Advisor polling access denied', { requestId: requestId });
            return { status: 'access_denied', error: 'Request does not belong to the current user session' };
        }
    ...
    function dataStore(requestId, toolName, result) {
        ...
        const summarySource = dataPayload.metadata.truncated ? {
            ...result,
            rows: dataPayload.rows,
            columns: dataPayload.columns
        } : result;
        const summary = generateDataSummary(summarySource, toolName);
        if (dataPayload.metadata.truncated) {
            summary.truncated = true;
            summary.originalRowCount = dataPayload.metadata.originalRowCount;
        }
        return {
            refId: refId,
            requestId: requestId,
    ...
    function dataLoad(requestId, refId) {
        ...
        try {
            ...
            const owner = parsed && parsed.metadata ? parsed.metadata.owner : null;
            if (owner && !isOwnerMatch(owner)) {
                log.audit('Advisor data access denied', { requestId: requestId, refId: refId });
                return null;
            }
    ...
    function dataFormatReferenceForPrompt(reference) {
        ...
        if (s.truncated) {
            lines.push(`Stored subset only: ${s.rowCount} of ${s.originalRowCount} rows. Re-run with narrower filters for exhaustive follow-up analysis.`);
        }
    ...
    function buildToolCacheKey(toolName, args) {
        ...
        return PREFIX.TOOL + `${owner.userId}_${owner.roleId}_${toolName}_${argsHash}`;
    }
    ...
    function getReferenceInfo(refId, requestId) {
        if (!refId || typeof refId !== 'string') {
            return null;
        }

        if (refId.startsWith(PREFIX.DASHBOARD)) {
            let cached = memoryCache[refId];
            if (cached && !isOwnerMatch(cached.owner)) {
                cached = null;
            }
            if (!cached) {
                cached = safeGet(refId);
            }
            if (!cached) return null;
            return {
                refId: refId,
                isDashboard: true,
                dashboardId: cached.dashboardId || null,
        ...
        return {
            ...
            truncated: data.metadata?.truncated === true,
            originalRowCount: data.metadata?.originalRowCount || data.metadata?.rowCount || null
        };
    }
