...
define([
    ...
    const NON_ADMIN_SAFE_TOOLS = new Set([
        'list_dashboards',
        'list_capabilities',
        'load_collection',
        'load_cached_data'
    ...
    function getDashboardIdForTool(toolName, args) {
        if (toolName && toolName.indexOf('dashboard_') === 0) {
            return toolName.replace('dashboard_', '');
        }

        if (toolName === 'load_collection' || toolName === 'load_cached_data') {
            const refId = args && (args.ref_id || args.refId);
            const requestId = args && args.request_id;
            if (refId && refId.indexOf('dash_') === 0) {
                const info = Cache.getReferenceInfo(refId, requestId);
                return info && info.dashboardId ? info.dashboardId : null;
            }
        }
    ...
    function checkToolAccess(toolName, args) {
        const dashboardId = getDashboardIdForTool(toolName, args);
        const subsidiaryId = normalizeNumericId(
            args && (args.subsidiary_id || args.subsidiaryId || args.subsidiary)
        );

        if (dashboardId) {
            if (!Permissions.hasPermission(dashboardId)) {
                return {
                    allowed: false,
                    message: 'You do not have permission to access the requested dashboard data.'
                };
            }
            if (subsidiaryId && !Permissions.hasSubsidiaryAccess(subsidiaryId)) {
                return {
                    allowed: false,
                    message: 'You do not have permission to access the requested subsidiary.'
                };
            }
            return { allowed: true };
        }
        ...
        if (toolName === 'load_cached_data') {
            const refId = args && args.ref_id;
            if (refId && refId.indexOf('dash_') !== 0) {
                return {
                    allowed: false,
                    message: 'Detailed query-cache follow-ups are restricted to administrators. Use dashboard follow-ups instead.'
                };
            }
        }

        if (NON_ADMIN_SAFE_TOOLS.has(toolName)) {
            return { allowed: true };
        }

        return {
            allowed: false,
            message: 'This advisor tool is restricted to administrators. Use a permitted dashboard analysis tool instead.'
        };
    }

    function isToolVisibleToCurrentUser(toolName) {
        const tool = ALL_TOOLS[toolName];
        if (!tool || tool.exposed === false) {
            return false;
        }
        return checkToolAccess(toolName, {}).allowed;
    }
...
AGGREGATE OPERATIONS: sum, avg, min, max, count`,
            parameters: {
                ...
                properties: {
                    ...
                    request_id: {
                        type: 'string',
                        description: 'Required for query result refs; optional for dashboard refs'
                    },
                    collection_name: {
                        type: 'string',
            ...
            execute: function(args) {
                ...
                const refInfo = Cache.getReferenceInfo(refId, requestId);
                ...
                try {
                    ...
                    if (isDashboardRef && args.collection_name) {
                        ...
                        if (!requestId) {
                            return {
                                success: false,
                                error: 'request_id is required for query result refs',
                                hint: 'Use the request_id that came with the original refId, or ask the advisor to re-run the query before drilling in.',
                                refId: refId,
                                tool: 'load_cached_data'
                            };
                        }
                        ...
                        if (args.group_by && args.aggregate_column) {
                            ...
                            return {
                                ...
                                truncated: refInfo && refInfo.truncated === true,
                                originalRowCount: refInfo ? refInfo.originalRowCount : null,
                                warning: refInfo && refInfo.truncated === true
                                    ? `This aggregation was computed from a stored subset of cached rows (${groupResult.totalRows || 'stored'} of ${refInfo.originalRowCount}). Re-run the original query with narrower filters for exhaustive analysis.`
                                    : null,
                                tool: 'load_cached_data'
                            };
                        }
                        ...
                        if (args.filter && typeof args.filter === 'object') {
                            ...
                            return {
                                ...
                                originalRowCount: refInfo ? refInfo.originalRowCount : null,
                                warning: refInfo && refInfo.truncated === true
                                    ? `This filter was applied to a stored subset of cached rows (${filterResult.totalRows} of ${refInfo.originalRowCount}). Re-run the original query with narrower filters for exhaustive analysis.`
                                    : null,
                                tool: 'load_cached_data'
                            };
                        }
                        ...
                        return {
                            ...
                            originalRowCount: refInfo ? refInfo.originalRowCount : null,
                            warning: refInfo && refInfo.truncated === true
                                ? `This cached result stores a subset of rows (${data.range.total} of ${refInfo.originalRowCount}). Re-run the original query with narrower filters for exhaustive analysis.`
                                : null,
                            tool: 'load_cached_data'
                        };
                    }
...
- "Help me understand your capabilities"`,
            ...
            execute: function(args) {
                ...
                for (const toolName in ALL_TOOLS) {
                    ...
                    if (tool.exposed === false || tool.category === 'internal' || !isToolVisibleToCurrentUser(toolName)) {
                        continue;
                    }
...
- Get GL line columns: { "record_type": "transactionaccountingline" }`,
    ...
    function getToolDefinitions() {
        ...
        for (const toolName in ALL_TOOLS) {
            const tool = ALL_TOOLS[toolName];
            if (!isToolVisibleToCurrentUser(toolName)) {
                continue;
            }
    ...
    function executeTool(toolName, args) {
        ...
        const access = checkToolAccess(toolName, normalizedArgs);
        if (!access.allowed) {
            Utils.auditLog('Tool access denied', {
                tool: toolName,
                args: normalizedArgs,
    ...
    function getTool(toolName) {
        if (!isToolVisibleToCurrentUser(toolName) && !Permissions.isAdmin()) {
            return null;
        }
    ...
    function listToolsByCategory() {
        return {
            discovery: Object.keys(DISCOVERY_TOOLS).filter(isToolVisibleToCurrentUser),
            data: Object.keys(DATA_TOOLS).filter(isToolVisibleToCurrentUser),
            dashboard: Object.keys(DASHBOARD_TOOLS).filter(isToolVisibleToCurrentUser),
            utility: Object.keys(UTILITY_TOOLS).filter(isToolVisibleToCurrentUser)
        };
    }
    ...
    function getToolManifest() {
        ...
        for (const toolName in ALL_TOOLS) {
            ...
            if (tool.exposed === false || !isToolVisibleToCurrentUser(toolName)) {
                continue;
            }
    ...
    function getToolListForPrompt() {
        ...
        for (const toolName in ALL_TOOLS) {
            ...
            // Skip internal/unexposed tools
            if (tool.exposed === false || !isToolVisibleToCurrentUser(toolName)) continue;
    ...
    function getToolDefinition(toolName) {
        const tool = ALL_TOOLS[toolName];
        if (!tool || !isToolVisibleToCurrentUser(toolName)) return null;
    ...
    function getToolSchemaText(toolName) {
        const tool = ALL_TOOLS[toolName];
        if (!tool || !isToolVisibleToCurrentUser(toolName)) return 'Unknown tool';
