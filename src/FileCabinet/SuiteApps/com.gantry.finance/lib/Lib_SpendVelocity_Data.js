...
define(['N/query', 'N/log', './Lib_Core', './Lib_Config'], function(query, log, Core, ConfigLib) {
    ...
    function getCurrencyInfo(subsidiaryId) {
        const safeSubsidiaryId = Core.normalizeInteger(subsidiaryId, { min: 1 });

        try {
            if (safeSubsidiaryId) {
                const subSql = `
                    SELECT C.symbol, C.name AS currency_name
                    FROM Subsidiary S
                    LEFT JOIN Currency C ON S.currency = C.id
                    WHERE S.id = ${safeSubsidiaryId}
                `;
                const subResults = safeQuery(subSql, 'currencyInfoSubsidiary');
                if (!isQueryError(subResults) && subResults.length > 0 && subResults[0].symbol) {
                    return {
            ...
            `;
            const results = safeQuery(sql, 'currencyInfoBase');
            if (!isQueryError(results) && results.length > 0 && results[0].symbol) {
                return {
            ...
            log.error('SpendVelocity Currency Info', e.message);
        }
    ...
    function getFiscalYearDates() {
        ...
        var fyStart = Core.buildFiscalYearStart(
            today,
            fiscalCalendar.fiscalYearStartMonth || 0,
            fiscalCalendar.fiscalYearStartDay || 1
        );
