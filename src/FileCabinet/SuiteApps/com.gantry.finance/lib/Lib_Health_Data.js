...
define(["N/search", "N/query", "N/log", "./Lib_Core", "./Lib_Config"], function (search, query, log, Core, ConfigLib) {
    ...
    function normalizeDateInput(value, fallback) {
        return Core.normalizeDateString(value) || fallback || null;
    }

    function normalizePositiveInt(value) {
        return Core.normalizeInteger(value, { min: 1 });
    }

    function normalizeMonths(value, fallback) {
        return Core.normalizeInteger(value, { min: 1, max: 36 }) || fallback;
    }

    function normalizeSegmentType(value) {
        const normalized = String(value || 'department').toLowerCase();
        return ['department', 'class', 'location'].indexOf(normalized) !== -1 ? normalized : null;
    }
    ...
    function getData(context) {
        try {
            ...
            const fiscalYearStartDay = fiscalCalendar.fiscalYearStartDay || 1;
            ...
            // Fiscal Context
            const fyStart = Core.buildFiscalYearStart(rangeEnd, fiscalYearStartMonth, fiscalYearStartDay);
            ...
            const deptPayload = departments.map(dept => {
                ...
                return {
                    ...
                    breakeven: { targetGMPct: Core.round2(dTargetGM), breakevenMonthlyRevenue: dBreakeven === null ? null : Core.round2(dBreakeven) },
                    forecast: { runRateRevenueFy: Core.round2(dRunRate) },
                    yoy: { revenueDeltaPct: dYoy === null ? null : Core.round2(dYoy) },
                    healthScore: dScore,
                    analysis: narrative,
            ...
            return {
                ...
                company: {
                    ...
                    breakeven: { targetGMPct: Core.round2(compTargetGM), breakevenMonthlyRevenue: compBreakeven === null ? null : Core.round2(compBreakeven) },
                    forecast: { runRateRevenueFy: Core.round2(compRunRate) },
                    yoy: { revenueDeltaPct: compYoyRevPct === null ? null : Core.round2(compYoyRevPct) },
    ...
    function handleRequest(params) {
        ...
        const priorStartDate = normalizeDateInput(params.priorStartDate);
        const priorEndDate = normalizeDateInput(params.priorEndDate);
        const subsidiaryId = normalizePositiveInt(params.subsidiaryId);
        const accountId = normalizePositiveInt(params.accountId);
        const departmentId = normalizePositiveInt(params.departmentId);
        const segmentId = normalizePositiveInt(params.segmentId);
        const months = normalizeMonths(params.months, 12);
        const periodsForward = normalizeMonths(params.periodsForward, 6);
        const segmentType = normalizeSegmentType(params.segmentType);
        const segmentIds = Core.normalizeIntegerList(params.segmentIds, { min: 1 });
        ...
        switch (subAction) {
            case 'account_transactions':
                if (!accountId || !startDate || !endDate) return invalid('Valid accountId, startDate, and endDate are required');
                return {
                    status: 'success',
            ...
            case 'department_pl':
                if (!departmentId || !startDate || !endDate) return invalid('Valid departmentId, startDate, and endDate are required');
                return {
                    status: 'success',
            ...
            case 'segment_profitability':
                if (!segmentType || !startDate || !endDate) return invalid('Valid segmentType, startDate, and endDate are required');
                return {
                    status: 'success',
            ...
            case 'budget_variance':
                if (!startDate || !endDate) return invalid('Valid startDate and endDate are required');
                return {
                    status: 'success',
            ...
            case 'account_monthly_trend':
                if (!accountId) return invalid('Valid accountId is required');
                return {
                    status: 'success',
            ...
            case 'margin_bridge':
                if (!startDate || !endDate || !priorStartDate || !priorEndDate) return invalid('Valid current and prior date ranges are required');
                return {
                    status: 'success',
            ...
            case 'account_anomalies':
                return {
                    ...
                    data: detectAccountLevelAnomalies(
                        months,
                        subsidiaryId,
                        isFinite(parseFloat(params.anomalyThreshold)) ? parseFloat(params.anomalyThreshold) : 2.0
                    )
                };
            ...
            case 'segment_monthly_trend':
                if (!segmentType || !segmentId) return invalid('Valid segmentType and segmentId are required');
                return {
                    status: 'success',
                    data: getSegmentMonthlyTrend(
                        segmentType,
                        segmentId,
                        normalizeMonths(params.months, 6),
                        endDate,
                        subsidiaryId
                    )
                };
            ...
            case 'balance_sheet':
                return {
                    status: 'success',
                    data: fetchBalanceSheetData(subsidiaryId)
                };
    ...
    function getAccountTransactions(accountId, startDate, endDate, subsidiaryId) {
        const safeAccountId = normalizePositiveInt(accountId);
        const safeStartDate = normalizeDateInput(startDate);
        const safeEndDate = normalizeDateInput(endDate);
        const safeSubsidiaryId = normalizePositiveInt(subsidiaryId);
        if (!safeAccountId || !safeStartDate || !safeEndDate) {
            return [];
        }
        ...
        const sql = `
            ...
            WHERE 
                t.posting = 'T'
                AND tal.posting = 'T'
                AND tal.account = ${safeAccountId}
                AND t.trandate >= TO_DATE('${safeStartDate}', 'YYYY-MM-DD')
                AND t.trandate <= TO_DATE('${safeEndDate}', 'YYYY-MM-DD')
                ${safeSubsidiaryId ? `AND t.subsidiary = ${safeSubsidiaryId}` : ''}
            ORDER BY 
    ...
    function getSegmentProfitabilityForId(segmentType, segmentId, startDate, endDate, subsidiaryId) {
        const safeSegmentId = normalizePositiveInt(segmentId);
        const safeStartDate = normalizeDateInput(startDate);
        const safeEndDate = normalizeDateInput(endDate);
        const safeSubsidiaryId = normalizePositiveInt(subsidiaryId);
        if (!safeSegmentId || !safeStartDate || !safeEndDate) {
            return null;
        }
        ...
        const sql = `
            ...
            WHERE 
                t.posting = 'T'
                AND tal.posting = 'T'
                AND ${segmentField} = ${safeSegmentId}
                AND a.accttype IN ('Income', 'OthIncome', 'COGS', 'Expense', 'OthExpense')
                AND t.trandate >= TO_DATE('${safeStartDate}', 'YYYY-MM-DD')
                AND t.trandate <= TO_DATE('${safeEndDate}', 'YYYY-MM-DD')
                ${safeSubsidiaryId ? `AND t.subsidiary = ${safeSubsidiaryId}` : ''}
    ...
    function getScoreOnly() {
        try {
            ...
            var fiscalYearStartDay = fiscalCalendar.fiscalYearStartDay || 1;
            ...
            var startDate = Core.buildFiscalYearStart(endDate, fiscalYearStartMonth, fiscalYearStartDay);
