...
define(["N/record", "N/search", "N/query", "N/log", "N/runtime", "./Lib_Dashboard_Registry"], function (record, search, query, log, runtime, DashboardRegistry) {
    ...
    function getSecretFields(configName) {
        return SECRET_FIELDS_BY_CONFIG[configName] || [];
    }

    function preserveStoredSecrets(configName, incomingData) {
        const secretFields = getSecretFields(configName);
        if (!secretFields.length || !incomingData || typeof incomingData !== 'object') {
            return incomingData;
        }

        const existingConfig = getStoredConfiguration(configName);
        const merged = Object.assign({}, incomingData);

        secretFields.forEach(function(fieldId) {
            const incomingValue = merged[fieldId];
            if (incomingValue === '' || incomingValue === null || incomingValue === undefined) {
                if (existingConfig[fieldId]) {
                    merged[fieldId] = existingConfig[fieldId];
                } else {
    ...
    function redactSecretsForApi(configName, config) {
        const secretFields = getSecretFields(configName);
        ...
        secretFields.forEach(function(fieldId) {
            secretStatus[fieldId] = !!sanitized[fieldId];
            sanitized[fieldId] = '';
        });

        sanitized._secretStatus = secretStatus;
        return sanitized;
    }
    ...
    function save(data, configName) {
        ...
        try {
            const dataToSave = preserveStoredSecrets(configName, data || {});
            let recordId = null;
            const configSearch = search.create({
                type: CONFIG_RECORD_TYPE,
                filters: [
    ...
    function getConfigForApi(configName) {
        configName = configName || 'cashflow';
        const config = redactSecretsForApi(configName, getStoredConfiguration(configName));
